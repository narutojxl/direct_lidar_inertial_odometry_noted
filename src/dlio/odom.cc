...
dlio::OdomNode::OdomNode(ros::NodeHandle node_handle) : nh(node_handle) {
  ...
  this->odom_pub     = this->nh.advertise<nav_msgs::Odometry>("odom", 1, true); //每帧imu propagate的odometry
  this->pose_pub     = this->nh.advertise<geometry_msgs::PoseStamped>("pose", 1, true); //每帧imu propagate pose
  this->path_pub     = this->nh.advertise<nav_msgs::Path>("path", 1, true); //每帧imu propagate pose path
  this->kf_pose_pub  = this->nh.advertise<geometry_msgs::PoseArray>("kf_pose", 1, true); //每个关键帧的位姿
  this->kf_cloud_pub = this->nh.advertise<sensor_msgs::PointCloud2>("kf_cloud", 1, true); //每个关键帧scan已转换到map下
  this->deskewed_pub = this->nh.advertise<sensor_msgs::PointCloud2>("deskewed", 1, true); //每帧scan已转换到map下
...
void dlio::OdomNode::publishPose(const ros::TimerEvent& e) {
  ...
  this->odom_ros.twist.twist.linear.x = this->state.v.lin.w[0]; //TODO(jxl): 不应该是body下的vel吗, this->state.v.lin.b ?
...
void dlio::OdomNode::preprocessPoints() {
  ...
  if (this->deskew_) {//去畸变
    ...
    this->deskewPointcloud(); //当前帧每个points坐标去畸变, 且转换到map下
    ...
    this->deskewed_scan = deskewed_scan_; //用imu预测值把当前帧每个点转到map下
    this->deskew_status = false;
  }
...
void dlio::OdomNode::deskewPointcloud() {
  ...
  std::vector<double> timestamps; //每个点的时间从小到大排列
  ...
  //当前帧中间点的时间作为scan_stamp

  // don't process scans until IMU data is present
  if (!this->first_valid_scan) {//第1帧scan  //认为第一帧scan时, 车辆处于静止状态
    if (this->imu_buffer.empty() || this->scan_stamp <= this->imu_buffer.back().stamp) {
      return;
    }
  ...
  std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>> frames; //用imu数据propagate出当前帧每个点时刻robot pose
  frames = this->integrateImu(this->prev_scan_stamp, this->lidarPose.q, this->lidarPose.p,
  ...
  this->T_prior = frames[median_pt_index]; //用imu数据预测的当前帧中间点时刻robot pose
...
void dlio::OdomNode::setInputSource() {
  this->gicp.setInputSource(this->current_scan); //去完畸变后,用imu预测值转换到map下
...
void dlio::OdomNode::callbackPointCloud(const sensor_msgs::PointCloud2ConstPtr& pc) {
  ...
  // Preprocess points
  this->preprocessPoints();//当前帧每个points坐标去畸变, 且转换到map下
  ...
  // Compute Metrics  对于每帧scan都会计算metrics
  ...
  this->setInputSource(); //设置去畸变后且转换到map下的source点云和计算covariance

  // Set initial frame as first keyframe
  if (this->keyframes.size() == 0) {//第一帧scan作为第一个keyframe
    this->initializeInputTarget();
    this->main_loop_running = false;
    this->submap_future =
      std::async( std::launch::async, &dlio::OdomNode::buildKeyframesAndSubmap, this, this->state ); //this->state = 0
  ...
  this->getNextPose(); //scan-to-map对当前帧中间点时刻robot pose进行correction  

  // Update current keyframe poses and map
  this->updateKeyframes(); //判断当前帧是否关键帧
  ...
  this->prev_scan_stamp = this->scan_stamp;//当前帧中间points的时间
  ...
  if (this->densemap_filtered_) {
    published_cloud = this->current_scan; //this->deskewed_scan voxel filter
  } else {//false
    published_cloud = this->deskewed_scan; //用imu预测值把当前帧每个点转到map下
  }
  this->publish_thread = std::thread( &dlio::OdomNode::publishToROS, this, published_cloud, this->T_corr );//global correction tf
...
void dlio::OdomNode::callbackImu(const sensor_msgs::Imu::ConstPtr& imu_raw) {
  ...
  sensor_msgs::Imu::Ptr imu = this->transformImu( imu_raw ); //首先把acc, gyro转换到base_link下
  ...
  if (!this->imu_calibrated) {
    ...
    if ((imu->header.stamp.toSec() - this->first_imu_stamp) < this->imu_calib_time_) {//小于3s, 收集数据
      ...
      if (this->gravity_align_) {//计算初始时刻姿态角
    ...
    double dt = imu->header.stamp.toSec() - this->prev_imu_stamp; //TODO(jxl): 第一次运行的时候忘记 prev == curr
    ...
    this->mtx_imu.lock();
    this->imu_buffer.push_front(this->imu_meas); //front时间戳最大. 当容量超过时,每插入一个新元素在最前面会挤掉最后一个元素
    this->mtx_imu.unlock();
...
void dlio::OdomNode::getNextPose() {
  ...
  //this->T_prior: 用imu数据预测的当前帧中间点时刻robot pose
  //this->T: scan-to-map校正后当前帧中间点时刻robot pose
  ...
  this->updateState(); //用gicp scan-to-map的(r, p)对imu预测的结果(r, p, v, ba, bg)做修正(融合)
...
dlio::OdomNode::integrateImu(double start_time, //上一帧scan中间点的时间
                             Eigen::Quaternionf q_init, Eigen::Vector3f p_init, //上一帧中间点的位姿, 作为每帧scan的位姿?
                             Eigen::Vector3f v_init, //上一帧中间点的速度
                             const std::vector<double>& sorted_timestamps) {//当前帧每个点的时时间,从小到大排列
  ...
  //在imu buffer中找到时刻在(start_time, sorted_timestamps.back() )间的两个迭代器位置

  // Backwards integration to find pose at first IMU sample
  const ImuMeas& f1 = *begin_imu_it;
  const ImuMeas& f2 = *(begin_imu_it+1);  
  
  // f1   start_time   f2   ...    sorted_timestamps.back()   end_imu_it
  // |         |        |                 |                        |
  // =====================================================================> time  
  //[q_init, p_init, v_init]: start_time时刻的状态
  ...
  q_init = Eigen::Quaternionf (//first order integration,  q(v) = e(v/2) ~ [1, v/2], 欧拉公式扩展公式
  ...
  //q先更新, v, p后更新. v, p之间不区分顺序. 因为p更新前需要计算/hat{j_i}, paper equ(4)
  //此刻[q_init, p_init, v_init]: f1时刻的状态
  //用imu数据propagate出当前帧每个点时刻robot pose
...
void dlio::OdomNode::propagateState() {
  ...
  this->state.q.vec() += 0.5 * dt * tmp.vec(); //TODO(jxl): 为何不使用integrateImu()那种方式去更新q?
...
void dlio::OdomNode::updateState() {
  ...
  std::lock_guard<std::mutex> lock( this->geo.mtx ); 
  //TODO(jxl): 尽管加了锁, imu propagate函数暂时暂停, 但是 this->state对应的时刻至少已经是这帧最后一个points之后的时刻, 而this->lidarPose是中间点时刻, 两个不是同一个时间点

  Eigen::Vector3f pin = this->lidarPose.p; //this->lidarPose: 中间点时刻robot pose
...
sensor_msgs::Imu::Ptr dlio::OdomNode::transformImu(const sensor_msgs::Imu::ConstPtr& imu_raw) {
  ...
  if (dt == 0) { dt = 1.0/200.0; }//默认imu 200hz
  ...
  //TODO(jxl): imu acc转到base_link下时, 需要考虑杆臂
...
void dlio::OdomNode::updateKeyframes() {
  ...
  for (const auto& k : this->keyframes) {
    ...
    //判断当前帧是否关键帧, 应该用this->lidarPose和每一个history关键帧计算diff, 尽管this->state和this->lidarPose时间上离地很近,误差很小
  ...
  Eigen::Vector3f closest_pose = this->keyframes[closest_idx].first.first; //position
  Eigen::Quaternionf closest_pose_r = this->keyframes[closest_idx].first.second; //orientation
  ...
  if (newKeyframe) {
    ...
    std::unique_lock<decltype(this->keyframes_mutex)> lock(this->keyframes_mutex);
    this->keyframes.push_back(std::make_pair(std::make_pair(this->lidarPose.p, this->lidarPose.q), this->current_scan)); //用imu预测值把当前帧每个点转到map下
    this->keyframe_timestamps.push_back(this->scan_header_stamp);
    this->keyframe_normals.push_back(this->gicp.getSourceCovariances()); //用imu预测值把当前帧每个点转到map下的cov
    this->keyframe_transformations.push_back(this->T_corr);
    lock.unlock();
...
void dlio::OdomNode::pushSubmapIndices(std::vector<float> dists, int k, std::vector<int> frames) {
  ...
  std::priority_queue<float> pq; //默认从大到小排列
...
void dlio::OdomNode::buildSubmap(State vehicle_state) {
  ...
  this->pushSubmapIndices(ds, this->submap_knn_, keyframe_nn); //submap_kf_idx_curr: {得到距离当前state knn个最近关键帧的indexs}

  // get convex hull indices
  this->computeConvexHull(); //keyframe_convex: 在处理过的关键帧中找凸包关键帧的索引

  // get distances for each keyframe on convex hull
  std::vector<float> convex_ds; //当前state距离每个凸包关键帧的距离
  for (const auto& c : this->keyframe_convex) {
    convex_ds.push_back(ds[c]);
  }

  // get indices for top kNN for convex hull
  this->pushSubmapIndices(convex_ds, this->submap_kcv_, this->keyframe_convex); //submap_kf_idx_curr += {在凸包关键帧中找kcv个最近凸包关键帧indexs}

  // get concave hull indices
  this->computeConcaveHull(); //keyframe_concave: 在处理过的关键帧中找凹包关键帧的索引

  // get distances for each keyframe on concave hull
  std::vector<float> concave_ds; //当前state距离每个凹包关键帧的距离
  for (const auto& c : this->keyframe_concave) {
    concave_ds.push_back(ds[c]);
  }

  // get indices for top kNN for concave hull
  this->pushSubmapIndices(concave_ds, this->submap_kcc_, this->keyframe_concave); //submap_kf_idx_curr += {在凹包关键帧中找kcc个最近凹包关键帧indexs}
  ...
  if (this->submap_kf_idx_curr != this->submap_kf_idx_prev){
    ...
    for (auto k : this->submap_kf_idx_curr) {
      ...
      lock.lock();
      *submap_cloud_ += *this->keyframes[k].second; //点云和cov已经转换到map下了
      lock.unlock();
    ...
    this->submap_cloud = submap_cloud_; //计算跟submap有关的一些变量
...
void dlio::OdomNode::buildKeyframesAndSubmap(State vehicle_state) {
  ...
  for (int i = this->num_processed_keyframes; i < this->keyframes.size(); i++) {
    pcl::PointCloud<PointType>::ConstPtr raw_keyframe = this->keyframes[i].second; //用imu预测值把当前帧每个点转到map下
    std::shared_ptr<const nano_gicp::CovarianceList> raw_covariances = this->keyframe_normals[i]; //用imu预测值把当前帧每个点转到map下的cov
    Eigen::Matrix4f T = this->keyframe_transformations[i]; //global correction tf: this->T_corr
    lock.unlock();
    ...
    lock.lock();
    this->keyframes[i].second = transformed_keyframe; //用scan match的修正量再进行一次correction, 点云和cov精确地转换到map坐标系下
...
void dlio::OdomNode::pauseSubmapBuildIfNeeded() {
  ...
  this->submap_build_cv.wait(lock, [this]{ return !this->main_loop_running; }); //false: 直接返回; true: 阻塞,等待notify
}
