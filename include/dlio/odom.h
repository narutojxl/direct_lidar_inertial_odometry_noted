...
private:
  ...
  std::vector<std::pair<std::pair<Eigen::Vector3f, Eigen::Quaternionf>,
                        pcl::PointCloud<PointType>::ConstPtr>> keyframes; //[i]: <每帧中间点robot pose scan-to-map修正后的值T, 用T转换到map下点云>
  std::vector<ros::Time> keyframe_timestamps; //[i]: 每帧scan msg time
  std::vector<std::shared_ptr<const nano_gicp::CovarianceList>> keyframe_normals; //[i]: 用T转换到map下点云的cov
  std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>> keyframe_transformations; //[i]: global correction tf
  ...
  // Point Clouds
  pcl::PointCloud<PointType>::ConstPtr original_scan; //raw scan
  ...
  pcl::ConvexHull<PointType> convex_hull; //凸包检测
  pcl::ConcaveHull<PointType> concave_hull; //凹包检测
  ...
  // Transformations
  Eigen::Matrix4f T, T_prior, T_corr; //每帧中间点时刻robot pose (scan-to-map修正后的值, imu估计的初值, global correction tf).
  ...
  boost::circular_buffer<ImuMeas> imu_buffer; //imu callback 压入数据 
  std::mutex mtx_imu;
  ...
  struct Velocity {
    Frames lin; //robot body's vel;   map's vel
    Frames ang; //robot body's omega; map's omega 
  };
  ...
  Pose lidarPose; //每帧中间点时刻robot pose
  Pose imuPose;
